...
public class BottomRight extends IsoscelesRightTriangle {
	 ...
	 * 作成日：2023/05/19
	 */
	public BottomRight(int higthNumber) {
		// 親クラスのコンストラクタを呼び出し、初期化
		super(higthNumber);
	}
	/*
	 * 関数名：drawFigure
	 * 概要：右下直角の二等辺三角形を描画するメソッド
	 * 引数：なし
	 * 返り値：なし
	 * パラメーターの説明：なし
	 * 作成者：OIkawaKouhei
	...
	public void drawFigure() {
		...
		for (int firstCount = 1; firstCount <= getHigth(); firstCount++) {
			 ...
			 */
			for (int thirdNumber = 0; thirdNumber < firstCount; thirdNumber++) {
				//「*」を表示し、直角三角形をつくる
				System.out.print('*');
			}
	 ...
	 * 概要：直角三角形の情報を表示
	 * 引数：なし
	 * 返り値："BottomRight(高さ：" + getHigth() + ",底辺：" + getBottom() + ")";
	 * パラメーターの説明：なし
	 * 作成者：OIkawaKouhei
	 * 作成日：2023/05/19
	 */
	...
	public String toString() {
		// "BottomRight(高さ：" + getHigth() + ")"を返す
		return "BottomRight(高さ：" + getHigth() + ")";
	}
